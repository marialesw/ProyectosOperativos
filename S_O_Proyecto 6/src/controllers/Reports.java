...
public class Reports{
	...
	public void generateReport( MyDoubleList<Particion> listaParticiones,
			...
			MyQueue<Condensacion> queueCondensations, MyQueue<Particion> queueFinish,MyQueue<Particion> listReadyProcess, String name) {
		...
		try {
			...
			reportNoAccepted(tabla3, queueCondensations);
	...
	private void reportNoAccepted(PdfPTable tabla3, MyQueue<Condensacion> queueCondensations) {
		tabla3.addCell("Nombre condensacion");
		tabla3.addCell("Nombre particion");
		tabla3.addCell("Concatenacion");
		tabla3.addCell("Tamaño final");
		MyQueue<Condensacion> myQueue = queueCondensations;
		while(!myQueue.isEmpty()) {
			Condensacion condensation = myQueue.get();
			tabla3.addCell(""+ condensation.getNameCondensation());
			tabla3.addCell(""+ condensation.getparticion().getNameParticion());
			if (condensation.getParticion3() != null) {
				tabla3.addCell(""+ condensation.getParticion1().getNameParticion() + " + " + condensation.getParticion2().getNameParticion()
						+ " + " + condensation.getParticion3().getNameParticion());
			}else {
				tabla3.addCell(""+ condensation.getParticion1().getNameParticion() + " + " + condensation.getParticion2().getNameParticion());
			}
			tabla3.addCell(""+ condensation.getparticion().getSizeTotalParticion());
		}
	}
	...
	private Paragraph encabezado() {
		...
		parrafo1.add("Reporte creado para simular la ejecución de procesos que tiene un Sistema Operativo"
				+ " \n El siguiente informe aplica al tema de Multiprogramación con particiones variables: "
				+ "condensación de huecos del almacenamiento en la multiprogramación con particiones variables \n\n");
		return parrafo1;
	}
